...
with st.container():
    ...
    with col3:
        st.markdown("#### Speed")
        speed = st.number_input("Enter Speed", min_value=2.5, max_value=5.0, value=None, step=0.1, placeholder="Enter Speed (rpm)")
    
    # Center-align the button
    _, button_col, _ = st.columns([1, 2, 1])
    with button_col:
        # Button to predict
        predict_btn = st.button("Predict ROP & Soil Type", key="predict_button")
